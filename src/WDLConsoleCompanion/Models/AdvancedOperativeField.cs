...
internal sealed class AdvancedOperativeField : INotifyPropertyChanged
{
    ...
    public required string Value { get => _value; set { _value = Normalize(value); OnChanged(nameof(Value)); OnChanged(nameof(ResolvedName)); OnChanged(nameof(SelectedOption)); } }
    ...
    public MetadataOption? SelectedOption
    {
        get => Options.FirstOrDefault(option => option.Value.Equals(Normalize(Value), StringComparison.OrdinalIgnoreCase));
